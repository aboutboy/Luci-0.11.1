...
function m.parse(map)
	if m:formvalue("cbid.wireless.%s.__toggle" % wdev:name()) then
		...
		luci.http.redirect(luci.dispatcher.build_url("admin/wireless"))
		return
	end
	Map.parse(map)
end
...
s = m:section(TypedSection,"wifi-iface",translate("Base"))
ifsection = s
s.addremove = false
s.anonymous = true
...
----------------------------------------------------------------------------------------------------------------
s:option(Value, "ssid", translate("<abbr title=\"Extended Service Set Identifier\">NetworkSSID</abbr>"))
...
if hwtype == "atheros" then
	...
	wdssep = s:option(Flag, "wdssep", translate("Separate WDS"))
	wdssep:depends({mode="ap-wds"})

	s:option(Flag, "doth", "802.11h")
	hidden = s:option(Flag, "hidden", translate("Hide <abbr title=\"Extended Service Set Identifier\">ESSID</abbr>"))
--*	hidden:depends({mode="ap"})
--	hidden:depends({mode="adhoc"})
--	hidden:depends({mode="ap-wds"})
--	hidden:depends({mode="sta-wds"})
	isolate = s:option(Flag, "isolate", translate("Separate Clients"),
	 translate("Prevents client-to-client communication"))
--*	isolate:depends({mode="ap"})
	s:option(Flag, "bgscan", translate("Background Scan"))

	mp = s:option(ListValue, "macpolicy", translate("MAC-Address Filter"))
	mp:value("", translate("disable"))
	mp:value("allow", translate("Allow listed only"))
	mp:value("deny", translate("Allow all except listed"))
	...
	s:option(Value, "rate", translate("Transmission Rate"))
	s:option(Value, "mcast_rate", translate("Multicast Rate"))
	s:option(Value, "frag", translate("Fragmentation Threshold"))
	s:option(Value, "rts", translate("RTS/CTS Threshold"))
	s:option(Value, "minrate", translate("Minimum Rate"))
	s:option( Value, "maxrate", translate("Maximum Rate"))
	s:option(Flag, "compression", translate("Compression"))

	s:option(Flag, "bursting", translate("Frame Bursting"))
	s:option( Flag, "turbo", translate("Turbo Mode"))
	s:option(Flag, "ff", translate("Fast Frames"))

	s:option(Flag, "wmm", translate("WMM Mode"))
	s:option( Flag, "xr", translate("XR Support"))
	s:option(Flag, "ar", translate("AR Support"))

	local swm = s:option(Flag, "sw_merge", translate("Disable HW-Beacon timer"))
	swm:depends({mode="adhoc"})

	local nos = s:option( Flag, "nosbeacon", translate("Disable HW-Beacon timer"))
	nos:depends({mode="sta"})
	nos:depends({mode="sta-wds"})

	local probereq = s:option(Flag, "probereq", translate("Do not send probe responses"))
	probereq.enabled  = "0"
	probereq.disabled = "1"
end
]]--
...
-------------------------------------------------------------------------------------------------------------------------------------
		...
		encr:value("psk2", "WPA2-PSK", {mode="ap"})
...
--[[	eaptype = s:option(ListValue, "eap_type", translate("EAP-Method"))
	...
	clientcert = s:option( FileUpload, "client_cert", translate("Path to Client-Certificate"))
...
s = m:section(TypedSection, "wifi-device",translate("Advanced"))
s.addremove = false
s.anonymous = true
...
if hwtype == "mac80211" then
	...
	--if htcaps then
		...
		--noscan = s:option(Flag, "noscan", translate("Force 40MHz mode"),
	...
	--s:option(Value, "distance", translate("Distance Optimization"),
	--	translate("Distance to farthest network member in meters."))

	-- external antenna profiles
	local eal = iw and iw.extant
	if eal and #eal > 0 then
		ea = s:option(ListValue, "extant", translate("Antenna Configuration"))
...
if hwtype == "atheros" then
	...
	s:option(Flag, "diversity", translate("Diversity")).rmempty = false

	if not nsantenna then
		ant1 = s:option( ListValue, "txantenna", translate("Transmitter Antenna"))
		ant1.widget = "radio"
		...
		ant2 = s:option(ListValue, "rxantenna", translate("Receiver Antenna"))
		ant2.widget = "radio"
		...
		local ant = s:option(ListValue, "antenna", translate("Transmitter Antenna"))
		ant:value("auto")
		ant:value("vertical")
	...
	s:option(Value, "distance", translate("Distance Optimization"),
		translate("Distance to farthest network member in meters."))
	s:option(Value, "regdomain", translate("Regulatory Domain"))
	--s:option(Value, "country", translate("Country Code"))
	s:option(Flag, "outdoor", translate("Outdoor Channels"))
...
if hwtype == "broadcom" then
	...
	s:option(Flag, "frameburst", translate("Frame Bursting"))

	s:option(Value, "distance", translate("Distance Optimization"))
	--s:option(Value, "slottime", translate("Slot time"))

	s:option(Value, "country", translate("Country Code"))
	s:option(Value, "maxassoc", translate("Connection Limit"))
end
...
if hwtype == "prism2" then
	s:option(Value, "txpower", translate("Transmit Power"), "att units").rmempty = true

	s:option(Flag, "diversity", translate("Diversity")).rmempty = false

	s:toption(Value, "txantenna", translate("Transmitter Antenna"))
	s:option(Value, "rxantenna", translate("Receiver Antenna"))
end
